...
local _G = getfenv(0)
local WclPlayerScore = _G.LibStub("AceAddon-3.0"):NewAddon("WclPlayerScore-Vanilla", "AceTimer-3.0")


SLASH_WP_Commands1 = "/wcl"
SlashCmdList["WP_Commands"] = function(msg)
	print("WCLPlayerScore-Vanilla Version: " .. addonVersion)
end
...
local function loadScoreDB()
	...
	for k, v in ipairs(ScoreDB2) do
		if v == GetRealmName() then LoadAddOn("WclPlayerScore-Vanilla_DB2")
		end
	end
	for k, v in ipairs(ScoreDB3) do
		if v == GetRealmName() then LoadAddOn("WclPlayerScore-Vanilla_DB3")
		end
	end
...
Addon_EventFrame:SetScript("OnEvent",
	function(self, event, addon)
		if addon == "WclPlayerScore-Vanilla" then
